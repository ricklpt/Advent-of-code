...
namespace Advent_of_code._2020
{
	public class Day1
	{
		...
		public Day1()
		{
			...
			var count2 = input.Where(Part2).Count();
			Console.WriteLine(count2);
		}

		private bool Part1(string input)
		{
			var match = Regex.Match(input, @"(?<firstdigit>\d+)-(?<seconddigit>\d+) (?<letter>.): (?<password>\w+)");
			var firstdigit = match.Groups["firstdigit"].Value;
			var seconddigit = match.Groups["seconddigit"].Value;
			var letter = match.Groups["letter"].Value;
			var password = match.Groups["password"].Value;

			var output = Regex.Replace(password, $"[^{letter}]", "");
			var pattern =  "^" + letter + "{" + firstdigit + "," + seconddigit + "}$";
			var isMatch = Regex.IsMatch(output, pattern);
			return isMatch;
		}

		private bool Part2(string input)
		{
			var match = Regex.Match(input, @"(?<firstdigit>\d+)-(?<seconddigit>\d+) (?<letter>.): (?<password>\w+)");
			var firstdigit = Convert.ToInt32(match.Groups["firstdigit"].Value);
			var seconddigit = Convert.ToInt32(match.Groups["seconddigit"].Value);
			var letter = Convert.ToChar(match.Groups["letter"].Value);
			var password = match.Groups["password"].Value;

			var firstchar = password[firstdigit - 1];
			var secondchar = password[seconddigit - 1];

			return (firstchar == letter)^(secondchar == letter);
		}
	}
}
